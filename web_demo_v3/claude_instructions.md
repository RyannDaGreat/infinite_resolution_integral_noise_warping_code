...
- **Stereo stars (added 2026-07-05; report §13 has the algorithm + proofs)**:
  ...
  **Cull Orphans** (default OFF, setting `cullOrphans`, SETTINGS_VERSION 12):
  hides stars visible in only ONE eye's merged view (binocular-rivalry
  sparkle). Design: both eyes' merges process the same 2N candidates under a
  CANONICAL index (L-star j → j, R-star j → N+j; StarUniforms.flags bit 0
  tells each merge whether its OWN stream is the right eye), so mergeSelect
  writes candidate-indexed mergedPos/Meta plus a 2-bit eye-visibility mask
  (mergeMaskBuf, atomicOr; tail counters [2N]=L, [2N+1]=R, [2N+2]=shared,
  counted by the second pass via atomicOr's prior bits). The render draws a
  fixed 2N quads and the vertex shader degenerates candidates whose mask
  lacks this eye's bit — or, with Cull Orphans on, any mask ≠ 3. This
  RETIRED the drawIndirect compaction (freed the binding that kept
  mergeSelect at the 8-storage-buffer limit); per-eye render uniforms
  (starRenderUniformBufR) carry eyeBit 1/2, mono uses eyeBit 0 = no mask
  read. __starStats gains `shared`; measured: 98% shared at default IPD,
  EXACTLY 100% at IPD 0 (coincident eyes — the correctness sanity check),
  93% at IPD 0.5; survivor counts unchanged (pre-filter). `IPD` slider (0–0.5 world
